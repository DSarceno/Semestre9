...
\frame{
	...
	\begin{enumerate}[a)]
		...
		\onslide<3->{\item La magnetización efectiva $M(x)$ en el plato está definida por $B(x) - B_a = 4\pi M(x)$. Demuestre, en CGS, que $4\pi M(x) = -B_a (1/8\lambda ^2)(\delta ^2 - 4x^2)$, para $\delta \ll \lambda$.}
	\end{enumerate}
...
\frame{
	...
	\onslide<1->{\textit{(b)} Para encontrar la magnetización efectiva y tomar en cuenta que $\lambda \gg \delta$, expandimos en taylor los cosenos hiperbólicos
		...
		$$ \cosh{\delta /2\lambda} = 1 + \frac{1}{2} \frac{\delta ^2}{4\lambda} + \cdots . $$}
...
\frame{
	...
	\textbf{Efecto de un espacio de aire. } Discuta el efecto de un espacio de aire entre las placas de unin measurement capacitor y un dielectrico en la medición de constantes dieléctricas grandes. Cuál es la constante dieléctrica aparente más grande posible si el espacio de aire tiene un espesor de $10^{-3}$ del total del espesor? La presencia de espacios de aire pueden distorcionar la medición de constantes dieléctricas grandes.
